...
@Component({
  selector: 'app-same-day-driver-invoice',
  templateUrl: './driver-invoice.component.html',
  styleUrls: ['./driver-invoice.component.scss']
})
export class DriverInvoiceComponent implements OnInit {
  ...
  displayedColumns: string[] = ['status', 'duration', 'km', 'price', 'created_at'];
  ...
  getResumeReport(_loading) {
    ...
    this.financialService.getDriverResume(this.monthRef).subscribe(
      x => {
        if (x.status == "success") {
